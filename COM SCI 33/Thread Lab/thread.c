...
void *histo_1(void *vargp)
{
	...
	/* performed a simple what-if? experiment and moved the code outside: */
    /* for some reason moving outisde of the loop, it runs 115x faster ?! */
	/* inside the loop: 15808 ms ----- outside the loop: 137 ms ??????????*/
	// pthread_mutex_lock(&mutex); // P(mutex)
	for (j=ind; j<ind+STEP; ++j)
	{
		/* use this implementation, don't know if other way is allowed */
		pthread_mutex_lock(&mutex); // P(mutex)
		global_histogram[data[j] % BUCKET_SIZE]++;
...
void *histo_2(void *vargp)
{
	...
	/* from thread.h, both arrays are same size --> use in same manner */
	for (j=ind; j<ind+STEP; ++j)
	{
		/* by using %, we remove overflow (for data[j] > BUCKET_SIZE) */
		pthread_mutex_lock(&locks[data[j] % BUCKET_SIZE]); // P(mutex)

		global_histogram[data[j] % BUCKET_SIZE]++;
		
		pthread_mutex_unlock(&locks[data[j] % BUCKET_SIZE]); // V(mutex)
	}
}
...
void *histo_3(void *vargp
	...
	int** p; // Declaration for our pointer (based on declaration)

	/* in this case, our poitner is our bucket, and value is ++, or 1 */
	for (j=ind; j<ind+STEP; ++j)
	{
		p = &global_histogram[data[j] % BUCKET_SIZE];
...
void *histo_4(void *vargp)
{
	...
	/* use hint: each thread performs histogram in own local array */
	unsigned int test_histogram[BUCKET_SIZE];

	/* initialize our local array */
	for (int i = 0; i < BUCKET_SIZE; ++i)
		test_histogram[i] = 0;

	/* perform histogram operation in own local array */
	for (j=ind; j<ind+STEP; ++j)
		test_histogram[data[j] % BUCKET_SIZE]++;

	/* use hint: add up for final sums: */
	for (int i = 0; i < BUCKET_SIZE; ++i)
	{
		/* now use implementation of task 1 */
		pthread_mutex_lock(&mutex); 

		/* synchronization for adding for final sum */
		global_histogram[i] = test_histogram[i] + global_histogram[i]; 
