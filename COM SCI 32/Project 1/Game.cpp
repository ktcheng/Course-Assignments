...
{   
    for (;;)
    {
        ...
        else if (playerMove == "h") // Implementation for history command
        {
            m_arena->history().display(); // Displays history grid
            std::cout << "Press enter to continue.";

            std::string outputMsg = "";
            getline(std::cin, outputMsg);

            clearScreen(); // After pressing enter, will return to game screen
            m_arena->display("");
        }
